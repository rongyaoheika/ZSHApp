...
- (void)setupSubviews {
    ...
    self.navigationItem.title = @"推流设置";
    
    CGFloat retractX = 10;
    CGFloat viewWidth = KScreenWidth - retractX * 2;
    
    self.publisherURLTextField = [[UITextField alloc] init];
    self.publisherURLTextField.frame = CGRectMake(retractX, KNavigationBarHeight, viewWidth - 100, kRealValue(40));
    ...
    noticeLabel.textColor = KZSHColorRGB(175, 175, 175);
    ...
    self.publisherButton.center = CGPointMake(KScreenWidth / 2, KScreenHeight - 40 - KNavigationBarHeight - KBottomHeight);
...
- (void)setupParamData {
    ...
    audioProfileModel.segmentTitleArray = @[@"AAC_LC",@"HE_AAC",@"HE_AAC_V2",@"AAC_LD"];
    ...
    AlivcParamModel *videoOnlyModel = [[AlivcParamModel alloc] init];
    videoOnlyModel.title = NSLocalizedString(@"video_only_push_streaming", nil);
    videoOnlyModel.defaultValue = 0.0;
    videoOnlyModel.reuseId = AlivcParamModelReuseCellSwitch;
    videoOnlyModel.switchBlock = ^(int index, BOOL open) {
    ...
    self.dataArray = [NSMutableArray arrayWithObjects:resolutionModel, qualityModeModel,targetBitrateModel, minBitrateModel, initBitrateModel, audioBitrateModel, audioSampelModel, fpsModel, minFPSModel, gopModel, reconnectDurationModel, reconnectTimeModel, orientationModel, audioChannelModel, audioProfileModel, mirrorModel, audiOnly_encodeModeModel, videoOnlyModel,autoFocus_FlashModel, beauty_cameraTypeModel, asyncModel, watermarkModel,beautyModeModel, beautyWhiteModel, beautyBuffingModel, beautyRuddyModel, beautyCheekPinkModel, beautyThinFaceModel, beautyShortenFaceModel, beautyBigEyeModel, nil];
}
