...
- (void)setupSubViews {
    ...
    self.listTableView = [[UITableView alloc] initWithFrame:(CGRectMake(0, 0, KScreenWidth, KScreenHeight))
...
- (void)setupData {
    ...
    self.dataArray = [[NSArray alloc] initWithObjects: NSLocalizedString(@"live_basic_function", nil), NSLocalizedString(@"live_basic_function_old", nil), NSLocalizedString(@"license_declaration", nil), nil];
}
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if (indexPath.row == 0) {
        ...
        // 老接口 AlivcLiveSession
        AlivcLiveConfigViewController *configurationVC = [[AlivcLiveConfigViewController alloc] init];
