...
#ifdef DEBUG
        [_fTextField setText:VID];
#else
        [_fTextField setText:@"cedd0fb0b6594293be29dd0fa3f646c7"];
#endif
...
-(UITextField *)tTextField{
    if (!_tTextField) {
        ...
        [_tTextField setText:ACCESS_KEY_ID];
...
-(UITextField *)thTextField{
    if (!_thTextField) {
        ...
        [_thTextField setText:ACCESS_KEY_SECRET];
...
-(UITextView *)fourthTextView{
    if (!_fourthTextView) {
        ...
        _fourthTextView.text = SECURITY_TOKEN;
    }
...
- (void)qualityRequest:(UIButton *)sender {
    ...
    source.vid = self.fTextField.text;
    ...
    NSString *vid = @"cedd0fb0b6594293be29dd0fa3f646c7";
...
-(void) updateQualityInfo:(NSArray<AliyunDownloadMediaInfo*>*)mediaInfos{
    dispatch_async(dispatch_get_main_queue(), ^{
        ...
        for (AliyunDownloadMediaInfo* info  in mediaInfos) {
            NSString* text = [segmentedArray objectAtIndex:info.quality];
