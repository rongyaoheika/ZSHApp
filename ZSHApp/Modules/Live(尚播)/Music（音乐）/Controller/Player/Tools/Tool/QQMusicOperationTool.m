...
#pragma mark 单首音乐的操作
//todo
- (void)playMusic:(MusicModel *)music{
    
    NSString *fileName = music.file_link;
...
- (void)setUpLockMessage{
    ...
    // 1.0 当前正在播放的歌曲信息
    // 获取当前播放歌曲的所有歌词信息
    NSArray *lrcMs = [QQLrcDataTool getLrcData:musicMessageModel.musicM.lrclink];
    // 获取当前播放的歌词模型
    ...
    // 1.1 字典信息
    NSString *songName = musicMessageModel.musicM.title;
    NSString *singerName = musicMessageModel.musicM.author;
    NSTimeInterval costTime = musicMessageModel.costTime;
    NSTimeInterval totalTime = musicMessageModel.totalTime;
    
    NSString *icon = musicMessageModel.musicM.pic_huge;
    if (icon) {
