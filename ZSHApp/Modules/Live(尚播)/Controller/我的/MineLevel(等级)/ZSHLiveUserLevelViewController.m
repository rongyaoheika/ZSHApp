//
//  ZSHLiveUserLevelViewController.m
//  ZSHApp
//
...
#import "ZSHLiveUserLevelViewController.h"

@interface ZSHLiveUserLevelViewController ()

@end

@implementation ZSHLiveUserLevelViewController
...
- (void)createUI{
    ...
    // 快速升级示意图
    UIImageView *upgradeImageView = [[UIImageView alloc] initWithImage:[UIImage imageNamed:@"grade_image_3"]];
    [self.view addSubview:upgradeImageView];
    [upgradeImageView mas_makeConstraints:^(MASConstraintMaker *make) {
        make.centerX.mas_equalTo(self.view);
        make.top.mas_equalTo(quickUpgradeLabel).offset(kRealValue(55));
        make.width.mas_equalTo(kRealValue(175));
        make.height.mas_equalTo(kRealValue(175));
    }];
    
    // 获取经验值提醒
    NSDictionary *getExpLabelDic = @{@"text":@"看直播，分享直播，刷礼物，就可以获得经验值",@"font": kPingFangRegular(12),@"textAlignment":@(NSTextAlignmentCenter),@"textColor":KZSHColor929292};
    UILabel *getExpLabel = [ZSHBaseUIControl createLabelWithParamDic:getExpLabelDic];
    [self.view addSubview:getExpLabel];
    [getExpLabel mas_makeConstraints:^(MASConstraintMaker *make) {
        make.centerX.mas_equalTo(upgradeImageView);
        make.top.mas_equalTo(upgradeImageView).offset(kRealValue(194.5));
        make.width.mas_equalTo(self.view);
        make.height.mas_equalTo(kRealValue(13));
    }];
    
    // 获取上限
    NSDictionary *expLimitLabelDic = @{@"text":@"获取上限：看直播1小时／天，分享3次／天，送礼无上限",@"font": kPingFangRegular(12),@"textAlignment":@(NSTextAlignmentCenter),@"textColor":KZSHColor929292};
    UILabel *expLimitLabel = [ZSHBaseUIControl createLabelWithParamDic:expLimitLabelDic];
    [self.view addSubview:expLimitLabel];
    [expLimitLabel mas_makeConstraints:^(MASConstraintMaker *make) {
        make.centerX.mas_equalTo(upgradeImageView);
        make.top.mas_equalTo(upgradeImageView).offset(kRealValue(217.5));
        make.width.mas_equalTo(self.view);
        make.height.mas_equalTo(kRealValue(13));
    }];
}
