...
- (UILabel *) noDataLabel
{
    if (_noDataLabel == nil) {
        ...
        [_noDataLabel setTextColor:KZSHColor929292];
        [_noDataLabel setFont:kPingFangLight(14)];
    }
    return _noDataLabel;
}
...
- (void) setCityArray:(NSArray *)cityArray
{
    ...
    for (int i = 0; i < cityArray.count; i ++) {
        ...
        if (i < self.arrayCityButtons.count) {
            ...
            [button.titleLabel setFont:kPingFangLight(14)];
            [button.layer setMasksToBounds:YES];
            [button.layer setCornerRadius:3.0f];
            [button.layer setBorderColor:KZSHColor929292.CGColor];
...
- (void) layoutSubviews
{
    ...
    float width = kRealValue(90);     // button最小宽度
    ...
    for (int i = 0; i < self.arrayCityButtons.count; i ++) {
        ...
        [button setFrame:CGRectMake(x, y, width, kRealValue(30))];
        if ((i + 1) % t == 0) {
            y += kRealValue(30) + 5;
            x = WIDTH_LEFT;
        }else {
...
+ (CGFloat) getCellHeightOfCityArray:(NSArray *)cityArray
{
    ...
    if (cityArray != nil && cityArray.count > 0) {
        ...
        h += (10 + (kRealValue(30) + 5) * (cityArray.count / t + (cityArray.count % t == 0 ? 0 : 1)));
    }
    else {
        h += 17;
    }
