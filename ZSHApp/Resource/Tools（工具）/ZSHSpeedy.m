...
@implementation ZSHSpeedy

+(id)zsh_chageControlCircularWith:(id)anyControl AndSetCornerRadius:(NSInteger)radius SetBorderWidth:(NSInteger)width SetBorderColor:(UIColor *)borderColor canMasksToBounds:(BOOL)can
{
...
+(id)zsh_setSomeOneChangeColor:(UILabel *)label SetSelectArray:(NSArray *)arrray SetChangeColor:(UIColor *)color
{
...
+ (CGSize)zsh_calculateTextSizeWithText : (NSString *)text WithTextFont: (UIFont *)textFont WithMaxW : (CGFloat)maxW {
...
#pragma mark - 下划线
+ (void)zsh_setUpAcrossPartingLineWith:(UIView *)view WithColor:(UIColor *)color
{
    ...
    [cellAcrossPartingLine mas_makeConstraints:^(MASConstraintMaker *make) {
        ...
        make.size.mas_equalTo(CGSizeMake(view.zsh_width, 1));
        
    }];
    
}
#pragma mark - 竖线
+ (void)zsh_setUpLongLineWith:(UIView *)view WithColor:(UIColor *)color WithHightRatio:(CGFloat)ratio;
{
    if (ratio == 0) { // 默认1
        ratio = 1;
    }
    ...
    [cellLongLine mas_makeConstraints:^(MASConstraintMaker *make) {
        ...
        make.size.mas_equalTo(CGSizeMake(1, view.zsh_height * ratio));
        
    }];
}
#pragma mark - 首行缩进
+ (void)zsh_setUpLabel:(UILabel *)label Content:(NSString *)content IndentationFortheFirstLineWith:(CGFloat)emptylen
{
...
#pragma mark - 设置圆角
+ (void)zsh_setUpBezierPathCircularLayerWith:(UIButton *)control :(CGSize)size
{
...
#pragma mark - 字符串加星处理
+ (NSString *)zsh_EncryptionDisplayMessageWith:(NSString *)content WithFirstIndex:(NSInteger)findex
{
    if (findex <= 0) {
        findex = 2;
...
+ (void)zsh_SetUpAlterWithView:(UIViewController *)vc Message:(NSString *)message Sure:(void(^)())sureBlock Cancel:(void(^)())cancelBlock
{
