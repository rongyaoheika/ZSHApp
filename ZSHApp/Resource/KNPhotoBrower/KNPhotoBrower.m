                    ...
                    if(items.url){ // 如果是网络图片
                        ...
                        [mgr diskImageExistsForURL:[NSURL URLWithString:items.url] completion:^(BOOL isInCache) {
                            if(!isInCache){
                                ...
                                [[mgr imageCache] queryDiskCacheForKey:items.url done:^(UIImage *image, SDImageCacheType cacheType) {
                                    if(image.images != nil){
                                        [weakS savePhotoToLocation:UIImagePNGRepresentation(image)]; // 将 gif 图片存入本地
                                    }else{
...
//                tempView.image = [[mgr imageCache] imageFromCacheForKey:items.url];
                tempView.image = [[mgr imageCache] imageFromMemoryCacheForKey:items.url];
            }
        }else{
